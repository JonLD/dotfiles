...
const GIT_COMPLETION_CACHE_VERSION = "v1"
const GIT_COMPLETION_CACHE_DIR = ($nu.data-dir | path join "git-completion-cache")

def "git-completion-cache mtime" [path: string] {
  if not ($path | path exists) {
    return 0
  }

  try {
    ls -D $path | get modified.0 | format date "%s" | into int
  } catch {
    0
  }
}

def "git-completion-cache repo-key" [] {
  let git_dir = (try {
    ^git rev-parse --git-dir | str trim
  } catch {
    ""
  })
  ...
  let git_dir_abs = ($git_dir | path expand)
  let repo_id = ($git_dir_abs | str replace -a -r '[^A-Za-z0-9._-]' '_')
  let signature = [
    $GIT_COMPLETION_CACHE_VERSION
    (git-completion-cache mtime ($git_dir_abs | path join "FETCH_HEAD"))
    (git-completion-cache mtime ($git_dir_abs | path join "packed-refs"))
    (git-completion-cache mtime ($git_dir_abs | path join "refs" "heads"))
    (git-completion-cache mtime ($git_dir_abs | path join "refs" "remotes"))
...
def "git-completion-cache get" [name: string] {
  let repo_key = (git-completion-cache repo-key)
  if $repo_key == null {
    return null
  }

  let cache_path = ($GIT_COMPLETION_CACHE_DIR | path join $'($repo_key.repo_id)-($name).nuon')
  ...
  if $cached.signature != $repo_key.signature {
    return null
  }
...
def "git-completion-cache set" [name: string, data: any] {
  let repo_key = (git-completion-cache repo-key)
  if $repo_key == null {
    return $data
  }

  if not ($GIT_COMPLETION_CACHE_DIR | path exists) {
    mkdir $GIT_COMPLETION_CACHE_DIR
  }

  let cache_path = ($GIT_COMPLETION_CACHE_DIR | path join $'($repo_key.repo_id)-($name).nuon')
  {
    signature: $repo_key.signature
    data: $data
  } | to nuon | save -f $cache_path

  $data
}

def "git-completion-cache get-or-populate" [name: string, loader: closure] {
  let cached = (git-completion-cache get $name)
  if $cached != null {
    return $cached
  }

  let data = (do $loader)
  git-completion-cache set $name $data
}

def "nu-complete git local refs metadata" [] {
  git-completion-cache get-or-populate "local-refs-metadata" {||
    ^git for-each-ref --format '%(refname:lstrip=2)%09%(objectname:short)%09%(upstream:remotename)%(upstream:track)%09%(contents:subject)' refs/heads
    | lines
    | parse "{ref}\t{obj}\t{upstream}\t{subject}"
  }
}

def "nu-complete git remote refs metadata" [] {
  git-completion-cache get-or-populate "remote-refs-metadata" {||
    ^git for-each-ref --format '%(refname:lstrip=2)%09%(objectname:short)%09%(contents:subject)' refs/remotes
    | lines
    | parse "{ref}\t{obj}\t{subject}"
  }
}

def "nu-complete git mergable sources data" [] {
  git-completion-cache get-or-populate "mergable-sources" {||
    let local = (nu-complete git local refs metadata)
      | each {|x|
        {
          value: $x.ref
          description: $'Branch, Local, ($x.obj) ($x.subject), (if ($x.upstream | is-not-empty) { $x.upstream } else { "no upstream" })'
        }
      }
      | insert style 'light_blue'

    let remote = (nu-complete git remote refs metadata)
      | each {|x|
        {
          value: $x.ref
          description: $'Branch, Remote, ($x.obj) ($x.subject)'
        }
      }
...
def "nu-complete git remote branches by remote" [remote: string] {
  if ($remote | is-empty) {
    return []
  }

  git-completion-cache get-or-populate $'remote-branches-($remote)' {||
...
def "nu-complete git available upstream" [] {
  git-completion-cache get-or-populate "available-upstream" {||
    ^git for-each-ref --format '%(refname:short)' refs/heads refs/remotes | lines
  }
}

def "nu-complete git remotes" [] {
  git-completion-cache get-or-populate "remotes" {||
    ^git remote --verbose
...
def "nu-complete git local branches" [] {
  git-completion-cache get-or-populate "local-branches" {||
...
def "nu-complete git remote branches with prefix" [] {
  git-completion-cache get-or-populate "remote-branches-with-prefix" {||
...
def "nu-complete git mergable sources" [] {
  let branches = (nu-complete git mergable sources data)
  {
    options: {
...
def "nu-complete git push" [context: string, position: int] {
  ...
  let remote_branches = (nu-complete git remote branches by remote $remote)
